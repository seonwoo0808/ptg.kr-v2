    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await fetch('/editor/submit', {
                ...
                body: JSON.stringify({ title, description: savedData.description, markdown: savedData.markdown, createdDate: savedData.createdDate }),
            });
            if (!res.ok) {
    ...
    return (
        ...
        <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
          <div className="sm:mx-auto sm:w-full sm:max-w-sm">
            ...
            <p className="mt-2 text-center text-sm/6 dark:text-gray-400
                text-gray-500">
              ...
              <div>
                {uploading ? (
                    <button
                        type="submit"
                        disabled
                        className="flex w-full justify-center rounded-md bg-indigo-600 px-3 py-1.5 text-sm/6 font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 opacity-50 cursor-not-allowed"
                    >
